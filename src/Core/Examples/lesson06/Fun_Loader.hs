...
import System.Exit
import Fun_Aux (dropGLsizei,ord2,toBinary,toDecimal,shiftLeft,make0)
...
type AwbfBitmap = (GLsizei,GLsizei,PixelData (Color4 GLubyte))
...
getBmData :: String -> (GLsizei,GLsizei) -> Maybe ColorList3 -> IO (PixelData (Color4 GLubyte))
getBmData bmString (bmW,bmH) invList = 
  let colorList = makeColorList bmString (bmW,bmH) in do
  --mapM_ (putStrLn . show) (addInvisiblity colorList invList)
  withArray [Color4 r g b a | (r,g,b,a) <- addInvisiblity colorList invList] $
    (\bmData -> return (PixelData RGBA UnsignedByte bmData))
